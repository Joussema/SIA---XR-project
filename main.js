...
function createTunnel() {
    ...
    for (let i = 0; i < 15; i++) {
        // Tunnel walls (rectangular cross-section)
        ...
        // Create hollow tunnel by using multiple boxes
        const leftWall = new THREE.Mesh(wallGeometry, wallMaterial);
        leftWall.position.set(-4, 0, -i * 4);
        leftWall.scale.set(0.5, 1, 1);
        scene.add(leftWall);
        
        const rightWall = new THREE.Mesh(wallGeometry, wallMaterial);
        rightWall.position.set(4, 0, -i * 4);
        rightWall.scale.set(0.5, 1, 1);
        scene.add(rightWall);
        
        const ceiling = new THREE.Mesh(wallGeometry, wallMaterial);
        ceiling.position.set(0, 2.5, -i * 4);
        ceiling.scale.set(1, 0.3, 1);
        scene.add(ceiling);
    }

    // Create neon light frames
    for (let i = 0; i < 15; i++) {
